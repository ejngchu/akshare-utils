...
# Feishu sync - 自选表 + 持仓表 + 现金表一体化同步
lark-cli auth login --domain base   # first-time only
python skill/scripts/feishu_sync.py --dry-run     # preview
python skill/scripts/feishu_sync.py                # execute (incremental update)
python skill/scripts/feishu_sync.py --force       # force update all records
python skill/scripts/feishu_sync.py --rate-limit 1.5   # custom write interval
python skill/scripts/feishu_sync.py --on-error abort   # stop on error (default: skip)
python skill/scripts/feishu_sync.py --verify        # verify field IDs match Feishu schema

# Feishu config sync (fetch field IDs from Feishu API)
python skill/scripts/feishu_config.py [--dry-run]

# Run tests
PYTHONPATH=skill/scripts pytest tests/ -v
PYTHONPATH=skill/scripts pytest tests/test_watchlist.py::TestClassifyCode -v  # single test class
```

## Architecture

Scripts are located in `skill/scripts/`:
...
│     │     ├── upsert_batch()   → batch upsert (500 records/batch)
│     │     └── verify_fields()   → verify field IDs match Feishu schema
│     ├── setup_signal_handlers() → SIGTERM/SIGINT graceful shutdown
│     └── add_common_args()       → shared argparse flags (--dry-run, --force, etc.)
│
├── feishu_sync.py        # 自选表 + 持仓表 + 现金表一体化同步
│                         # Step 1-4: 读取自选表 → 过滤 → 爬取 → 写入自选表
│                         # Step 5-7: 读取持仓表 → 合并价格 → 计算市值/收益 → 写入持仓表
│                         # Step 8:   读取现金表 → 汇总各账户余额
│
├── feishu_config.py      # Config management + Feishu API sync
│     # ~/.config/feishu-ledger/config.json   - base_token, table IDs, field IDs (auto-synced)
│     # ~/.cache/feishu-ledger/price_cache.json - persisted price cache
│     # skill/assets/config.json               - performance tuning (upsert_delay)

skill/assets/config.json  # Performance config only: upsert_delay (git-tracked)
tests/                    # pytest test suite (in project root)
```
...
- Configuration loaded from: `feishu_base_token`, `watchlist_table_id`, `holdings_table_id`, `trade_table_id`, `cash_table_id`, `upsert_delay`
